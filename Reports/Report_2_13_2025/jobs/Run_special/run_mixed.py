...
if args.use_softKMeans_top:
    mp = '/'.join([mainpath, '_'.join(['mixed', 'Kmeans', args.COMM_operator, '_'.join([str(i) for i in args.community_sizes]), 'opt_clusts', str(args.compute_optimal_clusters)])])
else:
    mp = '/'.join([mainpath, '_'.join(['mixed', args.COMM_operator, '_'.join([str(i) for i in args.community_sizes]), 'opt_clusts', str(args.compute_optimal_clusters)])])
    ...
    for sd in stdev:
        ...
        for connect in connect_types:
            ...
            for index2, it in enumerate(iters):
                ...
                sim_args.savepath = '/'.join([mainpath, 'graphs', 'mixed', dirname])+'/'
