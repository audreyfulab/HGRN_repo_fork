...
sim_args.nodes_per_super3 = (20,50)
...
mainpath = '/mnt/ceph/jarredk/HGRN_repo/Reports/Report_2_13_2025/Output/Intermediate_applications/SET_SPECIAL'
...
if args.use_softKMeans_top:
    mp = '/'.join([mainpath, '_'.join(['mixed_umbalanced', 'Kmeans', args.COMM_operator, '_'.join([str(i) for i in args.community_sizes]), 'opt_clusts', str(args.compute_optimal_clusters)])])
else:
    mp = '/'.join([mainpath, '_'.join(['mixed_umbalanced', args.COMM_operator, '_'.join([str(i) for i in args.community_sizes]), 'opt_clusts', str(args.compute_optimal_clusters)])])
    ...
    for sd in stdev:
        ...
        for connect in connect_types:
            ...
            for index2, it in enumerate(iters):
                ...
                sim_args.savepath = '/'.join([mainpath, 'graphs', 'mixed_unbalanced', dirname])+'/'
                args.sp = '/'.join([mp, dirname])+'/'
                
                print(f'results sp: {args.sp}')
                print(f'graph_readpath = {sim_args.savepath}')
                ...
                to_delete = [var for var in list(locals().keys()) 
                            if var not in ['args', 'sim_args', 'mainpath', 'it', 'index1', 'iters', 
