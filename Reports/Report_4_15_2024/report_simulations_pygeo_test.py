...
from pygeo_model import GATEgeo, CommClassifer, HCD
    ...
    for idx, value in enumerate(zip(grid1, grid2, grid3)):
        ...
        if idx == which_net:
            ...
            HCD_model_truth = HCD(nodes, attrib, hidden_dims=hd, 
                                  comm_sizes=comm_sizes, **kwargs).to(device)
            ...
            A_truth = torch.Tensor(true_adj_undi[:nodes,:nodes]).requires_grad_()
            A_rmat = torch.Tensor(rmat).requires_grad_()
            A_r02 = torch.Tensor(in_adj02).requires_grad_()
            A_r05 = torch.Tensor(in_adj05).requires_grad_()
            A_r07 = torch.Tensor(in_adj07).requires_grad_()
