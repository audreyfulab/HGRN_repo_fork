...
lp_main = 'C:/Users/Bruin/Documents/GitHub/HGRN_repo/Simulated Hierarchies/DATA/Toy_examples/Intermediate_examples/Results/'
tables = []

net_type = ['SM','SM','SF','SF','RG','RG']
connect = ['Disc','Full','Disc','Full','Disc','Full']
layers = [3,3,3,3,3,3]
...
for idx, case in enumerate(expansion):
    ...
    gam_value = case[-2]
    Aloss_off = case[-1]
    nm = '__gam_'+str(gam_value)+'_delt_'+str(1)+'_reso_'+str(resolu1[i])+'_'+str(resolu2[i])+'_lamda_'+str(lam1[i])+'_'+str(lam2[i])+'_TOAL_'+str(Aloss_off)
    print('case: {}'.format(nm))
    tab = pd.read_csv(lp_main+'Simulation_Results_'+case_nm+nm+'.csv')
    print(tab.shape)
    nets = tab.shape[0]
    ...
    Lambda_middle = np.repeat(lam1[i], nets)
    Lambda_top = np.repeat(lam2[i], nets)
    TOAL_loss = np.repeat(Aloss_off, nets)
    ...
    literal3 = [ast.literal_eval(i) for i in tab['Loss_Modularity']]
    ...
    literal4 = [ast.literal_eval(i) for i in tab['Loss_Clustering']]
    clustering_top = [ i[0] if len(i) < 2 else i[1] for i in literal3]
    clustering_middle = [i[0] if len(i) >1 else 'NA' for i in literal3]
    ...
    master_table.to_csv('C:/Users/Bruin/Documents/GitHub/HGRN_repo/Simulated Hierarchies/DATA/Toy_examples/Intermediate_examples/Results/MASTER_results.csv')
