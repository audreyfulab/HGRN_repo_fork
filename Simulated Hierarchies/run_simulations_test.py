...
def run_simulations(save_results = False, which_network = 0, which_graph = 0, resolu = [1,1],
                    epochs=100, updates=25, gam = 1, delt = 1, true_comm_layers = True,
                    comm_sizes = [150, 10], attention_act = 'LeakyReLU', loss_fn = 'Modularity',
                    **kwargs):
    ...
    for idx, value in enumerate(zip(grid1, grid2, grid3)):
        ...
        if idx == which_network:
            ...
            HCD_model_truth = HCD(nodes, attrib, comm_sizes=comm_sizes, attn_act=attention_act, **kwargs)
            HCD_model_rmat = HCD(nodes, attrib, comm_sizes=comm_sizes, attn_act=attention_act, **kwargs)
            HCD_model_r02 = HCD(nodes, attrib, comm_sizes=comm_sizes, attn_act=attention_act, **kwargs)
            HCD_model_r05 = HCD(nodes, attrib, comm_sizes=comm_sizes, attn_act=attention_act, **kwargs)
            HCD_model_r08 = HCD(nodes, attrib, comm_sizes=comm_sizes, attn_act=attention_act, **kwargs)
            ...
            for i in range(0, len(case_nm)):
                ...
                if i == which_graph:
                    ...
                    out = fit(Mods[i], X, Graphs[i], optimizer='Adam', epochs = epochs, 
                              ...
                              lr = 1e-4, gamma = gam, delta = delt, comm_loss=loss_fn, 
                              layer_resolutions = resolu, true_labels = target_labels, 
                              verbose=False, save_output=save_results, output_path=savepath, 
                              fs = 3, ns = 10)
